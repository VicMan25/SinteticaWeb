...
@WebServlet(name = "SvAgregarReserva", urlPatterns = {"/sv_agregarReserva"})
public class SvAgregarReserva extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            r.setEstado(estado == null ? "Pendiente" : estado);

            new GestionarReservas().registrarReserva(r);
            response.sendRedirect("reservas.jsp?ok=agregado");

        } catch (SQLException e) {
            if (e.getMessage() != null && e.getMessage().contains("disponible")) {
                response.sendRedirect("reservas.jsp?error=ocupada");
            } else {
                getServletContext().log("Error SQL al agregar reserva", e);
                response.sendRedirect("reservas.jsp?error=servidor");
            }
        } catch (IllegalArgumentException | NullPointerException e) {
            response.sendRedirect("reservas.jsp?error=campos");
        } catch (Exception e) {
            getServletContext().log("Error al agregar reserva", e);
            response.sendRedirect("reservas.jsp?error=servidor");
        }
    }
}
